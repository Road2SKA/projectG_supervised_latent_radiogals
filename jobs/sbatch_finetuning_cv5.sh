...
# Run finetuning for a single model dir (TARGET_PATH = where results land).
# Pass --cv-fold=N via FOLD_ARGS for f<1 runs; leave empty for f=1.
run_finetuning() {
    local TARGET_PATH="$1"
    local FOLD_ARGS="$2"   # e.g. "--cv-fold=3" or ""

    for MODE in 2 3; do
        METRICS_FILE="${TARGET_PATH}/data/classifiers/finetuning/initial_pure_cwNone0.0_mode${MODE}_lr${LR}_ep${EPOCHS}/finetuning_metrics.json"
